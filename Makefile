...
CFLAGS := $(shell [ -n "${DD_TRACE_DOCKER_DEBUG}" ] && echo -O0 -g || echo -O2) $(WALL_FLAGS)
...
test_c_disabled: export SIGNALFX_TRACE_CLI_ENABLED=0
test_c_disabled: export SIGNALFX_TRACE_DEBUG=1
...
ENV_OVERRIDE := $(shell [ -n "${DD_TRACE_DOCKER_DEBUG}" ] && echo DD_AUTOLOAD_NO_COMPILE=true) SIGNALFX_TRACE_CLI_ENABLED=true
TEST_EXTRA_INI ?=
...
test_web_drupal_7: global_test_run_dependencies
...
test_web_drupal_9_2: global_test_run_dependencies
