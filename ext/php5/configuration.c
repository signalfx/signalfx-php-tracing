...
void ddtrace_config_first_rinit() {
    ...
    if (strcmp(internal_functions_old, internal_functions_new) != 0) {
        ddtrace_log_errf(
            ...
            internal_functions_old, internal_functions_new);
    }
    efree(internal_functions_old);
...
bool ddtrace_config_integration_enabled(ddtrace_integration_name integration_name) {
    ...
    if (zend_hash_exists(get_DD_INTEGRATIONS_DISABLED(), "default", sizeof("default"))) {
        return integration->is_enabled();
    } else {
        // Deprecated as of 0.47.1
        return !zend_hash_exists(get_DD_INTEGRATIONS_DISABLED(), integration->name_lcase, integration->name_len + 1);
    }
}
