...
ZEND_BEGIN_ARG_INFO_EX(arginfo_sfxtrace_hex_to_u64, 0, 0, 1)
...
static PHP_GINIT_FUNCTION(signalfx_tracing) {
#ifdef ZTS
    UNUSED(TSRMLS_C);
#endif
    php_ddtrace_init_globals(signalfx_tracing_globals);
}
...
static PHP_MINIT_FUNCTION(signalfx_tracing) {
     ...
     * {{{ */
    Dl_info infos;
    zend_register_extension(&_dd_zend_extension_entry, signalfx_tracing_module_entry.handle);
    dladdr(ZEND_MODULE_STARTUP_N(signalfx_tracing), &infos);
...
void ddtrace_disable_tracing_in_current_request(void) {
    zend_ini_entry *ini = zai_config_memoized_entries[DDTRACE_CONFIG_SIGNALFX_TRACING_ENABLED].ini_entries[0];
...
static PHP_FUNCTION(root_span) {
    ...
    Z_TYPE_P(return_value) = IS_OBJECT;
    Z_OBJVAL_P(return_value) = root_span->span.obj_value;
    zend_objects_store_add_ref(return_value TSRMLS_CC);
}
...
static const zend_function_entry signalfx_tracing_functions[] = {
    ...
    DDTRACE_FE(sfxtrace_hex_to_u64, arginfo_sfxtrace_hex_to_u64),
...
void dd_read_distributed_tracing_ids(TSRMLS_D) {
    ...
    if (zai_read_header_literal("X_DATADOG_TRACE_ID", &trace_id_str) == ZAI_HEADER_SUCCESS) {
        if (trace_id_str.len != 1 || trace_id_str.ptr[0] != '0') {
            ...
            success = ddtrace_set_userland_trace_id_hex(&trace_zv TSRMLS_CC);
        }
    }
