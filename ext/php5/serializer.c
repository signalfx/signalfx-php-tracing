...
static void _serialize_meta(zval *el, ddtrace_span_fci *span_fci TSRMLS_DC) {
    ...
    zend_bool error = zend_hash_exists(Z_ARRVAL_P(meta), "sfx.error.message", sizeof("sfx.error.message")) ||
                      zend_hash_exists(Z_ARRVAL_P(meta), "sfx.error.kind", sizeof("sfx.error.kind"));
    if (error) {
