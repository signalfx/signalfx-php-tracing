...
#define HASH(...) (zend_array*)&zend_empty_array, FALSE,
...
#define HASH(getter_name, env_name, ...)                                                \
    do {                                                                                \
        pthread_mutex_lock(&ddtrace_memoized_configuration.mutex);                      \
        ddtrace_memoized_configuration.getter_name = ddtrace_get_hash_config(env_name); \
        ddtrace_memoized_configuration.__is_set_##getter_name = TRUE;                   \
        pthread_mutex_unlock(&ddtrace_memoized_configuration.mutex);                    \
    } while (0);
...
#define HASH(getter_name, ...) extern inline zend_array* getter_name(void);
