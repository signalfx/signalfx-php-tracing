...
function build_version() {
    ...
    docker run --rm -w /var/app -v $(pwd):/var/app circleci/php:${PHP_VERSION}${SUFFIX} cp tmp/build_extension/.libs/signalfx-tracing.so extensions/signalfx-tracing-${PHP_API}${SUFFIX}.so
}
...
function build_version_54() {
    ...
    docker run --rm -w /var/app -v $(pwd):/var/app datadog/docker-library:ddtrace_centos_6_php_5_4 cp tmp/build_extension/.libs/signalfx-tracing.so extensions/signalfx-tracing-${PHP_API}.so
}
