    ...
    protected function drupal7($rootSpan)
    {
        ...
        \DDTrace\trace_function('menu_execute_active_handler', function (SpanData $span) use ($rootSpan) {
            if (!empty($GLOBALS['user'])) {
                $user = $GLOBALS['user'];
                $rootSpan->meta['drupal.user.id'] = strval($user->uid);
        ...
        \DDTrace\trace_function('drupal_http_request', [
            'posthook' => function (SpanData $span, $args, $retval) {
                ...
                $sanitizedUrl = \DDTrace\Util\Normalizer::urlSanitize($url);
                $normalizedPath = \DDTrace\Util\Normalizer::uriNormalizeOutgoingPath($url);
                ...
                if ($retval->error) {
                    $span->meta[Tag::ERROR_TYPE] = 'drupal_http_request error';
        ...
        \DDTrace\trace_method(
            ...
            function (SpanData $span, $args) use ($rootSpan, $integration) {
                ...
                if ($user->isAuthenticated()) {
                    $rootSpan->meta['drupal.user.id'] = strval($user->id());
                    $rootSpan->meta['drupal.user.roles'] = implode(',', $user->getRoles());
                }
